...
The daemon resource access base URL defaults to the local daemon host and port. When the base URL comes from environment configuration, non-local base URLs require `DELIBERUM_RESOURCE_ACCESS_ALLOW_REMOTE=true`; public base URLs must use HTTPS. `DELIBERUM_RESOURCE_ACCESS_SIGNING_SECRET` can require HMAC-signed daemon access URLs; the signing key is runtime-only, the returned URL carries only expiry and signature query parameters, and safe posture reports only whether signing is configured.
...
The current access grant service redirects already-registered safe URL variants and serves explicitly registered base64 broker content. `GET /runtime/resource-access` reports this as safe posture metadata, including URL signing status, hosted-content preconditions, restart-continuity classes, and production hosting blockers without exposing URLs, bearer access ids, signing secrets, signatures, data refs, or content. HMAC-signed daemon access URLs do not fetch arbitrary local files, implement object-storage or CDN signed URL services, or replace production authorization, multi-user policy, or remote deployment policy. SQLite daemon persistence preserves resource broker registrations/content plus grant and revocation enforcement across daemon restarts. Without SQLite, broker content and grant state remain process-local.
