...
The daemon resource access base URL defaults to the local daemon host and port. When the base URL comes from environment configuration, non-local base URLs require `DELIBERUM_RESOURCE_ACCESS_ALLOW_REMOTE=true`; public base URLs must use HTTPS.
...
The current access grant service redirects already-registered safe URL variants and serves explicitly registered base64 broker content. `GET /runtime/resource-access` reports this as safe posture metadata, including hosted-content preconditions, restart-continuity classes, and production hosting blockers without exposing URLs, bearer access ids, data refs, or content. The access grant service does not fetch arbitrary local files, issue production signed URLs, or replace production authorization, multi-user policy, or remote deployment policy. SQLite daemon persistence preserves resource broker registrations/content plus grant and revocation enforcement across daemon restarts. Without SQLite, broker content and grant state remain process-local.
