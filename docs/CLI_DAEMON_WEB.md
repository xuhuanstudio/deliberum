...
Run workspace actions require the local daemon to be running; the Web UI does not provide public hosting, production identity, multi-user deployment, or Web-based secret-capturing provider setup yet. The default pages use user language and keep raw run/session/projection/ledger/event/proposal/runtime/resource/internal ids behind Advanced / Developer Mode.
