...
Run workspace actions require the local daemon to be running; the Web UI does not provide public hosting, production identity, or multi-user deployment. Connect AI can submit OpenAI-compatible API key, base URL, model, and structured review compatibility values to the local daemon so it writes a marker-delimited local env block and applies the setup to the current daemon process when possible; the response does not return saved secret values. Connect AI and New Discussion can also save non-secret participant model choices to a separate local daemon setup block, including discussion depth and model names, without saving API keys, base URLs, or provider config ids in that participant-choice block. The default pages use user language and keep raw run/session/projection/ledger/event/proposal/runtime/resource/internal ids behind Advanced / Developer Mode.
