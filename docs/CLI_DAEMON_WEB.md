...
Run workspace actions require the local daemon to be running; the Web UI does not provide public hosting, production identity, or multi-user deployment. The Setup / Models page can submit OpenAI-compatible API key, base URL, and model values to the local daemon so it writes a marker-delimited local env block; the response does not return saved secret values, the daemon loads the managed block at startup, and the daemon must be restarted before the new setup becomes active. The default pages use user language and keep raw run/session/projection/ledger/event/proposal/runtime/resource/internal ids behind Advanced / Developer Mode.
