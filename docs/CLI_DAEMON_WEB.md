...
For the normal local Web product path, prefer `corepack pnpm start:local` after
`corepack pnpm build`; it serves the built Web shell from the daemon and enables
the deterministic local preset demo path. For lower-level local development and
testing only, the built daemon can still be started directly with:
...
The extraction prompt requests exactly one JSON object with no surrounding prose or Markdown, and the parser remains strict: it accepts a raw JSON object, a single full fenced JSON object, or one unambiguous embedded JSON object when the provider wraps the only object in surrounding text. Multiple embedded objects, arrays, non-object JSON, invalid JSON, and schema-invalid objects still fail. If a provider response fails this structured JSON shape check, the extractor may make one corrective retry without including the rejected response text. Additional non-secret request options are available for local compatibility testing: `DELIBERUM_OPENAI_TOP_P`, `DELIBERUM_OPENAI_STREAM=true|false`, `DELIBERUM_OPENAI_FREQUENCY_PENALTY`, and `DELIBERUM_OPENAI_PRESENCE_PENALTY`. When streaming is enabled, the OpenAI-compatible client assembles Chat Completions SSE text deltas into a complete response before the existing participant, extraction, review, or finalization parser handles the result.

Small token limits are appropriate for participant connectivity smoke runs only. Provider-backed extraction, review, and finalization return structured JSON and need enough output budget to avoid truncated JSON; if a response starts like an object but is cut off before valid JSON can be parsed, the stage fails safely with malformed-response diagnostics rather than storing partial proposal material.
