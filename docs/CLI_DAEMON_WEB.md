...
Run workspace actions require the local daemon to be running; the Web UI does not provide public hosting, production identity, or multi-user deployment. The Setup / Models page can submit OpenAI-compatible API key, base URL, and model values to the local daemon so it writes a marker-delimited local env block and applies the setup to the current daemon process when possible; the response does not return saved secret values. The default pages use user language and keep raw run/session/projection/ledger/event/proposal/runtime/resource/internal ids behind Advanced / Developer Mode.
