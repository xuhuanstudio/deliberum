...
| 7. Default UI hides secrets and internal runtime data; Advanced can keep diagnostics. | Complete | `smoke:web-product-loop`, `smoke:web-release-readiness`, `smoke:web-entry`, `smoke:web-boundaries`, and `smoke:web-resilience` scan default views for secrets, provider values, env names, provider config ids, raw JSON, low-level id labels, and internal runtime language. The latest `smoke:web-boundaries` coverage also opens Setup / Models, Discussion Room, current conclusion, landing, and legacy Advanced panels to confirm diagnostics remain intentionally available only after Advanced is opened. |
