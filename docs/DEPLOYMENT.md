...
The daemon also exposes `GET /runtime/deployment-posture` as a no-store safe diagnostic. It is useful for checking whether a local/pre-production daemon is still bound to localhost, whether control-plane bearer auth is enabled, whether auth is using a legacy single token or a scoped registry, which store classes are process-memory versus configured, whether resource access grants survive restarts, whether built Web static assets are enabled, and which production-readiness blockers still apply. It does not change daemon configuration and is not a production authorization, identity, or multi-user deployment system.
