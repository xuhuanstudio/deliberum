...
- CLI daemon runtime profile status, comment-only env-template output, safe env block writing, local interactive setup wizard secret capture, safe profile-doctor diagnostics, safe setup-plan output, scoped daemon auth entry generation, deployment posture reads, ledger integrity reads, and resource access posture reads for local setup assistance;
- shared `@deliberum/client` setup-plan projection helpers for CLI/Web local setup summaries derived from safe runtime profile metadata, plus daemon deployment posture and ledger integrity response types and reads;
...
- daemon ledger integrity reporting for current event-store snapshots, session/event counts, hash coverage counts, and sequence ranges without exposing event payloads, event ids, configured paths, secrets, resource access material, URLs, or hosted content;
