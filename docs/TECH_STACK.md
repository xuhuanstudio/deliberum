...
- CLI daemon runtime profile status, comment-only env-template output, safe env block writing, local interactive setup wizard secret capture, safe profile-doctor diagnostics, safe setup-plan output, deployment posture reads, and resource access posture reads for local setup assistance;
