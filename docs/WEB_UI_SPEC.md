...
- `/runs/new` lets a user start a discussion from a guided prompt or sample walkthrough, choose demo or model-backed participants, and select a focused or broader model perspective depth when a provider is ready. Advanced JSON request details stay collapsed.
