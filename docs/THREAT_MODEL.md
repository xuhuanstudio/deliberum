...
- no public URL grants are generated by default;
- signed resource access grants use TTL and revocation;
- require explicit opt-in for env-configured non-local resource access base URLs and HTTPS for public access base URLs;
...
- persist and optionally export daemon operation audit records as normalized control-plane metadata only, with a local retention cap and safe principal metadata, without request bodies, headers, bearer tokens, raw WebGET tokens, raw resource access ids, provider secrets, or output payloads;
...
- keep tokens daemon-local and scoped to one WebGET session;
- sanitize logs;
...
- local-first;
- no public daemon binding by default;
- no wildcard CORS by default;
- no public resource exposure by default;
- no API keys or provider credentials in repo files or examples;
