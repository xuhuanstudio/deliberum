...
- persist daemon operation audit records as normalized control-plane metadata only, with a local retention cap, without request bodies, headers, bearer tokens, raw WebGET tokens, raw resource access ids, provider secrets, or output payloads;
