...
The first production blocker remains gate 4: provider setup, verification,
failure recovery, timeout, malformed output, and partial completion states are
handled in normal-user language. Rate-limited setup verification now has browser
evidence from `smoke:web-resilience`. Gates 2, 3, and 7 are closed for the v1.0
supported Web scope, so the next batch should target provider-specific timeout
...
1. Audit current recovery evidence for provider verification failures,
   provider timeouts, malformed structured output, failed stages,
