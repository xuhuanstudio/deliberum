...
Gates 1 through 9 and Gate 11 are complete for the current v1.0 supported
scope. The next numbered incomplete gate is Gate 10.

Recommended narrow batch:

1. Target Gate 10: no known normal-user blocker remains in install, startup,
   setup, verification, discussion start, continuation, conclusion review, or
   recovery.
2. Audit the supported outside-user path against current evidence from Gates 1
   through 9 and Gate 11.
3. If a blocker is reproduced, fix only the first Deliberum-side blocker.
4. If no blocker is reproduced, record the Gate 10 blocker audit without adding
