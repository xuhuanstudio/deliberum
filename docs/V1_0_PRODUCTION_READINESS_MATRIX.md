...
Gates 1 through 8 and Gate 11 are complete for the current v1.0 supported
scope. The next numbered incomplete gate is Gate 9.

Recommended narrow batch:

1. Target Gate 9: CI, tests, language lint, docs lint, product-loop smoke, Web
   smoke, and real-provider release-readiness evidence are green and current.
2. Run current local checks from a clean tree, inspect GitHub CI for the latest
   pushed commit, and verify whether real-provider release-readiness evidence is
   still current enough for v1.0.
3. If a check fails, fix only the first reproduced Deliberum-side blocker.
4. If checks pass and evidence is sufficient, record Gate 9 evidence without
   adding product or runtime behavior.
