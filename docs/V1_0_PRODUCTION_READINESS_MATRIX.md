...
The first production blocker is now gate 7: Model / Participant Management.
The default Web path explains current provider-backed roles, but it does not yet
let normal users edit multiple providers, multiple models, or role-specific
model assignments in a production-grade way.

Recommended narrow batch:

1. Inspect the existing Setup / Models state model and Web setup persistence
   limits.
2. Choose one smallest user-facing participant-management gap that blocks
   production use.
