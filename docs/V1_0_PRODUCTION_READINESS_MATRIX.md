...
| 7 | Model / Participant Management supports understandable provider/model/role readiness and editing. | `partial` | Setup / Models shows current participant readiness, explains that one verified OpenAI-compatible provider powers the model-backed participant roles together, and links role assignment back to the shared provider setup form for editing. The start page lets users set a per-discussion shared model override and optional first-response Perspective A/B/C model overrides without exposing provider secrets or low-level provider config ids; review roles still use the shared model. | The default Web path does not yet provide production-grade editing for multiple providers or review-role-specific model assignment. |
...
model-backed discussion. It also lets users customize first-response
Perspective A/B/C models for one discussion while keeping review roles on the
shared model, but it does not yet let normal users edit multiple providers or
review-role-specific model assignments in a production-grade way.
