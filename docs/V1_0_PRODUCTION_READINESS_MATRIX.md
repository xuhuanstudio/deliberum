...
discussion. It can also save those non-secret role model choices as local
service defaults for later discussions without storing API keys, base URLs, or
provider config ids in the role-default setup. It still does not let normal
users edit multiple providers, manage multiple named provider setups, or choose
from a richer participant/model management surface.
