...
- Stabilization hardening for persisted ledger loading, optional daemon event/run/resource-broker/resource-access/operation-audit persistence, operation audit retention and local JSONL rotation, optional daemon control-plane auth, safe daemon deployment posture diagnostics, local SQLite writer serialization, WebGET decoded submission safety, commit finalization, projection traceability, idempotency result consistency, SSE idempotent publish guards, WebGET context visibility, resource access base URL opt-in, and resource delivery safety.
...
- Full interactive secret capture and config-file writing.
...
- Remote operation audit shipping integrations.
