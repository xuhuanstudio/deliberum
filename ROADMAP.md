...
- Stabilization hardening for persisted ledger loading, local event integrity hashes, optional daemon event/run/resource-broker/resource-access/operation-audit persistence, operation audit retention and restart-safe durable audit ids, local JSONL rotation, HTTP export, optional daemon control-plane auth with scoped registry support, safe daemon deployment posture diagnostics, local SQLite writer serialization plus optional single-daemon process locking, WebGET decoded submission safety, commit finalization, projection traceability, idempotency result consistency, SSE idempotent publish guards, WebGET context visibility, resource access base URL opt-in, optional resource access URL signing, and resource delivery safety.
