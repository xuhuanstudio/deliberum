...
- CLI commands for sessions, batches, contributions, extraction proposals, proposal challenge/acceptance, process proposal lifecycle, final candidate/audit/outcome projection, frontier, objections, obligations, events, local daemon runtime profile status, env-template output, safe env block writing, profile-doctor diagnostics, safe setup-plan output, local daemon deployment posture reads, local daemon operation audit reads and JSONL export, optional daemon control-plane bearer auth, local daemon resource access posture reads and revocation, local daemon run orchestration, daemon resources/evidence projection reads, daemon-backed final lifecycle submissions, explicit accepted process proposal execution with read-only execution readiness, and daemon-redacted run event timeline/follow support.
...
- Full interactive secret capture.
