...
- CLI commands for sessions, batches, contributions, extraction proposals, proposal challenge/acceptance, process proposal lifecycle, final candidate/audit/outcome projection, frontier, objections, obligations, events, local daemon runtime profile status, env-template output, profile-doctor diagnostics, local daemon operation audit reads and JSONL export, optional daemon control-plane bearer auth, local daemon resource access posture reads and revocation, local daemon run orchestration, daemon resources/evidence projection reads, daemon-backed final lifecycle submissions, explicit accepted process proposal execution, and daemon-redacted run event timeline/follow support.
