...
function preserveEnvVars(names: readonly string[]): Map<string, string | undefined> {
  return new Map(names.map((name) => [name, process.env[name]]));
}

function restoreEnvVars(previousEnv: ReadonlyMap<string, string | undefined>): void {
...
describe("daemon API", () => {
  ...
  it("serves configured Web assets for browser navigation without overriding JSON API routes", async () => {
    ...
    try {
      ...
      const advancedRouteResponse = await daemonApp.app.request("/advanced", {
        headers: {
          Accept: "text/html,application/xhtml+xml"
        }
      });
      ...
      expect(advancedRouteResponse.status).toBe(200);
      expect(advancedRouteResponse.headers.get("content-type")).toContain("text/html");
      await expect(advancedRouteResponse.text()).resolves.toContain("Deliberum Web Shell");
  ...
  it("saves OpenAI-compatible setup through a safe local Web setup route", async () => {
    ...
    try {
      ...
      expect(envContent).toContain(`${OPENAI_COMPATIBLE_MODEL_ENV_VAR}=model-for-web-setup`);
      expect(envContent).toContain(`${OPENAI_COMPATIBLE_API_KEY_ENV_VAR}=${apiKey}`);
      expect(auditEvents).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            action: "runtime_openai_compatible_setup_save",
            method: "POST",
            route: "/runtime/setup/openai-compatible",
            statusCode: 201
          })
        ])
      );
  ...
  it("loads the Web-managed provider setup block when the daemon starts", async () => {
    ...
    let startedDaemon: ReturnType<typeof startDaemon> | undefined;
    ...
    try {
      ...
      await listening;
      const response = await startedDaemon.app.request("/runtime/profiles");
      const body = (await response.json()) as {
        profiles: Array<{
          id: string;
          enabled: boolean;
      ...
      if (startedDaemon) {
        await closeStartedDaemon(startedDaemon);
      }
      restoreEnvVars(previousEnv);
      rmSync(tempDir, {
        recursive: true,
        force: true
      });
  ...
  it("keeps existing process env ahead of the Web-managed provider setup block", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "deliberum-setup-"));
    const envFilePath = join(tempDir, ".env");
    const env: Record<string, string | undefined> = {
      [OPENAI_COMPATIBLE_BASE_URL_ENV_VAR]: "https://already-configured.example/v1"
    };
    ...
    try {
      ...
      expect(loadManagedDaemonSetupEnvFile({ envFilePath, env })).toEqual({
        filePath: envFilePath,
        loaded: true,
        appliedEnvVars: [OPENAI_COMPATIBLE_MODEL_ENV_VAR],
        skippedEnvVars: [OPENAI_COMPATIBLE_BASE_URL_ENV_VAR]
      });
      expect(env[OPENAI_COMPATIBLE_BASE_URL_ENV_VAR]).toBe(
        "https://already-configured.example/v1"
      );
      expect(env[OPENAI_COMPATIBLE_MODEL_ENV_VAR]).toBe("model-for-web-setup");
    } finally {
      rmSync(tempDir, {
        recursive: true,
        force: true
  ...
  it("preserves unmanaged local env files when Web setup appends its managed block", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "deliberum-setup-"));
    const envFilePath = join(tempDir, ".env");

    try {
      writeFileSync(envFilePath, "EXISTING_VALUE=keep\n", "utf8");
      const daemonApp = createDaemonApp({
        idGenerator: createIds(),
        clock,
        setupEnvFilePath: envFilePath
      ...
      expect(envContent).toContain("EXISTING_VALUE=keep\n");
      expect(envContent).toContain(OPENAI_COMPATIBLE_SETUP_ENV_BEGIN);
      expect(envContent).toContain(
        `${OPENAI_COMPATIBLE_BASE_URL_ENV_VAR}=https://api.example.test`
      );
  ...
  it("verifies OpenAI-compatible setup through a safe provider request", async () => {
    const apiKey = "sk-local-web-setup-secret";
    const fetch = createOpenAICompatibleFetch("ready");
    const daemonApp = createDaemonApp({
      idGenerator: createIds(),
      clock,
      openAICompatibleFetch: fetch,
    ...
    expect(init.headers.Authorization).toBe(`Bearer ${apiKey}`);
    expect(request.model).toBe("model-for-web-setup");
    expect(request.messages.at(-1)?.content).toBe("Reply with ready.");
    expect(responseText).not.toContain(apiKey);
    expect(responseText).not.toContain("ready");
    expect(auditEvents).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
    ...
    expect(auditText).not.toContain(apiKey);
    expect(auditText).not.toContain("api.example.test");
    expect(auditText).not.toContain("ready");
  });

  it("returns a safe OpenAI-compatible verification error without exposing secrets", async () => {
    const apiKey = "sk-local-web-setup-secret";
