...
5. Start and continue a discussion with AI.
   ...
   When setup is ready, use **Start focused discussion** for two AI/model viewpoints or **Start broader discussion** for three AI/model viewpoints. You can also open `/runs/new?participants=model-backed`. Write the discussion question, optionally set **First-response model** for viewpoints without their own model, set **Review role model** for Skeptic, Evidence checker, Risk reviewer, and Summary writer, and use **Customize viewpoint models** only when individual first-response viewpoints should use different models. Use **Save participant choices** to reuse these non-secret model choices through the local service for future discussions; Connect AI can also edit, save, clear, and summarize the saved discussion depth and role models without API keys, base URLs, or internal provider details. Create the discussion, open the room, then use **Continue discussion** to collect participant perspectives and review strongest options, still-unresolved points, what needs checking, risks, the current answer, and next steps.
