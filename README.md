...
- local CLI commands for sessions, batches, contributions, extraction proposals, process proposal lifecycle, final candidate/audit/outcome projection, projections, events, daemon runtime profile status, env-template output, safe env block writing, profile-doctor diagnostics, safe daemon setup-plan output, daemon deployment posture reads, daemon operation audit reads and JSONL export, optional daemon control-plane bearer auth, daemon resource access posture reads and revocation, daemon run orchestration, daemon resources/evidence projection reads with safe delivery and access audit history, daemon-backed final lifecycle submissions, and explicit accepted process proposal execution;
