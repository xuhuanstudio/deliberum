...
- participant adapter interface, fake/manual adapters, OpenAI-compatible adapter/profile, HTTP-template participant adapter/profile for sealed participant execution, package-level MCP-compatible tool participant adapter, OpenAI-compatible extraction/review/finalization components, and experimental WebGET adapter;
...
Deferred work includes production multi-writer coordination for durable daemon stores, broader primitive runner coverage and automated policy around accepted process proposals, production resource hosting posture, daemon MCP profile and tool execution policy, full interactive provider setup, production authorization or multi-user deployment, and larger benchmark datasets.
